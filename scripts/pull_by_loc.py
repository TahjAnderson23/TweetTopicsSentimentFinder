...
config.read('/Users/tahjanderson/TweetTopicsByCity/config.ini')
...
def find_sentiment(df):
    vectorizer = pickle.load(open('/Users/tahjanderson/TweetTopicsByCity/models/vectorizer.sav', 'rb'))
    classifier = pickle.load(open('/Users/tahjanderson/TweetTopicsByCity/models/LR_classifier.sav', 'rb'))
    ...
    pos_list = []
    neg_list = []
    pos_list = df[df["sentiment"] == 1]
    neg_list = df[df["sentiment"] == 0]
    
    print("The number of postivie sentiments found ",len(pos_list))
    print("The amount of negative sentiments found ",len(neg_list))
